...
// ============================================================================
// PHASE 3.4: ABSENSI MULTI-EVENT (ATTENDANCE) API ENDPOINTS
// ============================================================================

function resolveAttendanceTenantId(req: AuthenticatedRequest): string {
  if (req.user?.role === 'SUPER_ADMIN') {
    return (req.query.tenantId as string) || (req.body.tenantId as string) || req.user.tenantId || 'ten_darulmusthafa';
  }
  return req.user?.tenantId || 'ten_darulmusthafa';
}

// GET /api/attendance/stats - Statistik umum absensi tenant
apiRouter.get(
  '/attendance/stats',
  authenticate,
  ...
  enforceTenantIsolation,
  (req: AuthenticatedRequest, res: Response) => {
    const tenantId = resolveAttendanceTenantId(req);
    const stats = db.getAttendanceStats(tenantId);
    return res.json({
      success: true,
      data: stats,
    });
...
// --- 1. JENIS ABSENSI (ATTENDANCE TYPES) ---
...
apiRouter.get(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    const types = db.getAttendanceTypes(tenantId, { category, status });
    return res.json({
      success: true,
      count: types.length,
      data: types,
...
apiRouter.get(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const tenantId = resolveAttendanceTenantId(req);
    const item = db.getAttendanceTypeById(id, tenantId);
    if (!item) {
      return res.status(404).json({
        success: false,
        error: 'Jenis absensi tidak ditemukan.',
      });
    }
    return res.json({
      success: true,
...
apiRouter.post(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = db.createAttendanceType({
      tenantId,
      code,
      name,
      category,
    ...
    if (!result.success || !result.data) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal menambahkan jenis absensi.',
      });
    }
    ...
    db.addAuditLog({
      ...
      userName: req.user!.fullName,
      userRole: req.user!.role,
      action: 'ATTENDANCE_TYPE_CREATED',
      module: 'Absensi',
      entityName: 'Jenis Absensi',
      entityId: result.data.id,
      previousData: null,
    ...
    return res.status(201).json({
      success: true,
      data: result.data,
      message: `Jenis absensi '${result.data.name}' (${result.data.category}) berhasil ditambahkan.`,
    });
  }
);

// PUT /api/attendance/types/:id
apiRouter.put(
  '/attendance/types/:id',
  authenticate,
  requirePermission('attendance.manage'),
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    const existing = db.getAttendanceTypeById(id, tenantId);
    if (!existing) {
      return res.status(404).json({
        success: false,
        error: 'Jenis absensi tidak ditemukan.',
    ...
    const result = db.updateAttendanceType(id, tenantId, {
      code,
      name,
      category,
      status,
    ...
    if (!result.success || !result.data) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal memperbarui jenis absensi.',
      });
    }
    ...
    db.addAuditLog({
      ...
      userName: req.user!.fullName,
      userRole: req.user!.role,
      action: 'ATTENDANCE_TYPE_UPDATED',
      module: 'Absensi',
      entityName: 'Jenis Absensi',
      entityId: id,
      previousData: JSON.stringify(existing),
    ...
    return res.json({
      success: true,
      data: result.data,
      message: `Jenis absensi '${result.data.name}' berhasil diperbarui.`,
    });
  }
);

// DELETE /api/attendance/types/:id
apiRouter.delete(
  '/attendance/types/:id',
  authenticate,
  requirePermission('attendance.manage'),
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = db.deleteAttendanceType(id, tenantId);
    if (!result.success) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal menghapus jenis absensi.',
      });
    }
    ...
    db.addAuditLog({
      ...
      userName: req.user!.fullName,
      userRole: req.user!.role,
      action: 'ATTENDANCE_TYPE_DELETED',
      module: 'Absensi',
      entityName: 'Jenis Absensi',
      entityId: id,
      previousData: JSON.stringify(existing),
    ...
    return res.json({
      success: true,
      message: `Jenis absensi '${existing.name}' berhasil dihapus.`,
    });
  }
);

// --- 2. SESI ABSENSI (ATTENDANCE SESSIONS) ---
...
apiRouter.get(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    const tenantId = resolveAttendanceTenantId(req);
    const { classId, attendanceTypeId, date, status } = req.query as {
      classId?: string;
    ...
    const sessions = db.getAttendanceSessions(tenantId, {
      classId,
      attendanceTypeId,
      date,
      status,
...
apiRouter.get(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const tenantId = resolveAttendanceTenantId(req);
    const session = db.getAttendanceSessionById(id, tenantId);

    if (!session) {
      return res.status(404).json({
        success: false,
        error: 'Sesi absensi tidak ditemukan.',
      });
    }
...
apiRouter.post(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    const tenantId = resolveAttendanceTenantId(req);
    const { attendanceTypeId, classId, date, startTime, endTime, status } = req.body;

    const result = db.createAttendanceSession({
      tenantId,
      attendanceTypeId,
      classId,
      date,
    ...
    if (!result.success || !result.data) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal membuka sesi absensi baru.',
      });
    }
    ...
    return res.status(201).json({
      success: true,
      data: result.data,
      message: `Sesi absensi berhasil dibuat untuk kelas ${result.data.classGroup?.name || ''} tanggal ${result.data.date}.`,
    });
  }
);
...
apiRouter.put(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const tenantId = resolveAttendanceTenantId(req);
    const { attendanceTypeId, startTime, endTime, status } = req.body;

    const existing = db.getAttendanceSessionById(id, tenantId);
    if (!existing) {
      return res.status(404).json({
        success: false,
        error: 'Sesi absensi tidak ditemukan.',
    ...
    const result = db.updateAttendanceSession(id, tenantId, {
      attendanceTypeId,
      startTime,
      endTime,
      status,
    ...
    if (!result.success || !result.data) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal memperbarui sesi absensi.',
      });
    }
...
apiRouter.post(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = db.closeAttendanceSession(id, tenantId);
    if (!result.success || !result.data) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal menutup sesi absensi.',
      });
    }
    ...
    return res.json({
      success: true,
      data: result.data,
      message: `Sesi absensi untuk kelas ${result.data.classGroup?.name || ''} resmi ditutup (CLOSED).`,
    });
  }
);
...
apiRouter.delete(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = db.deleteAttendanceSession(id, tenantId);
    if (!result.success) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal menghapus sesi absensi.',
      });
    }
    ...
    return res.json({
      success: true,
      message: `Sesi absensi '${existing.attendanceType?.name || ''}' tanggal ${existing.date} berhasil dihapus.`,
    });
  }
);

// --- 3. REKAM PRESENSI KEHADIRAN (ATTENDANCE RECORDS) ---
...
apiRouter.get(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    const records = db.getAttendanceRecords(id, tenantId);
    return res.json({
      success: true,
...
// POST /api/attendance/sessions/:id/records - Bulk record attendance
apiRouter.post(
  '/attendance/sessions/:id/records',
  authenticate,
  requirePermission('attendance.create'),
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!Array.isArray(records) || records.length === 0) {
      return res.status(400).json({
        success: false,
        error: 'Daftar data presensi santri (records array) tidak boleh kosong.',
      });
    }

    const recordedBy = req.user?.fullName || 'Petugas Absensi';
    const result = db.recordAttendance(id, tenantId, records, recordedBy);

    if (!result.success) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal mencatat data absensi.',
      });
    }

    const updatedSession = db.getAttendanceSessionById(id, tenantId);
    ...
    db.addAuditLog({
      ...
      module: 'Absensi',
      entityName: 'Rekap Presensi',
      entityId: id,
      previousData: null,
      newData: JSON.stringify({ count: result.count, sessionId: id }),
      ipAddress: req.ip || '127.0.0.1',
      userAgent: req.headers['user-agent'] || 'Unknown',
    });
    ...
    return res.json({
      success: true,
      message: `Presensi berhasil disimpan untuk ${result.count} santri.`,
      data: {
        session: updatedSession,
        records: db.getAttendanceRecords(id, tenantId),
      },
    });
  }
);
...
apiRouter.put(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    const recordedBy = req.user?.fullName || 'Petugas Absensi';
    const result = db.updateAttendanceRecord(id, tenantId, { status, note }, recordedBy);

    if (!result.success || !result.data) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal mengubah catatan absensi.',
      });
    }
...
// DELETE /api/attendance/records/:id - Delete single record
apiRouter.delete(
  '/attendance/records/:id',
  authenticate,
  requirePermission('attendance.delete'),
  ...
  (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = db.deleteAttendanceRecord(id, tenantId);
    if (!result.success) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Gagal menghapus catatan presensi.',
      });
    }
