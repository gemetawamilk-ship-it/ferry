...
export interface DashboardData {
  ...
  stats: {
    totalSantri: number;
    kehadiranSantri: number;
    totalGuru: number;
    totalKaryawan: number;
    kehadiranStaf: number;
    keuangan: {
      tagihanBulanIni: number;
      pemasukanBulanIni: number;
      pengeluaranBulanIni: number;
      piutangSantri: number;
      kasTersedia: number;
    };
    pengasuhan: {
      santriSakit: number;
      izinKeluarHariIni: number;
      pelanggaranBulanIni: number;
      hafalanBulanIniJuz: number;
    };
    personal: {
      role: RoleType;
      santriBinaan?: number;
      jamMengajarMingguIni?: number;
      saldoWallet?: number;
      anakDipantau?: string;
    };
  };
...
export type GenderType = EmployeeGender;
export type EmployeeType = 'GURU' | 'MUSYRIF' | 'STAFF' | 'STAF' | 'STRUKTURAL' | 'LAINNYA';
export type EmployeeStatus = 'ACTIVE' | 'INACTIVE';
export type TeachingStatus = 'ACTIVE' | 'INACTIVE';
export type MusyrifStatus = 'ACTIVE' | 'INACTIVE';
export type EducationLevel = 'SMA' | 'D3' | 'S1' | 'S2' | 'S3' | 'PESANTREN' | string;
...
export type StudentParentRelationship = 'AYAH' | 'IBU' | 'WALI' | 'KAKAK' | 'KAKEK' | 'NENEK' | 'PAMEN' | 'LAINNYA';
...
export type ClassGenderType = ClassGender;
